    ...
    <div className="min-h-screen bg-surface-page lg:flex">
      <aside className="app-sidebar hidden lg:fixed lg:inset-y-0 lg:left-0 lg:z-sticky lg:flex lg:w-64 lg:flex-col lg:overflow-y-auto">
        <Link to="/" className="flex items-center gap-3 px-7 py-7" aria-label="Sainik School Tracker home">
          ...
          <span>
            <span className="block text-sm font-semibold text-text-primary">Sainik School</span>
            <span className="mt-0.5 block text-[10px] font-semibold uppercase tracking-[0.2em] text-text-tertiary">Project tracker</span>
          </span>
        </Link>
        <div className="px-5 pt-8">
          <p className="px-3 text-[10px] font-semibold uppercase tracking-[0.2em] text-text-tertiary">Workspace</p>
        ...
        <div className="mt-auto px-7 pb-7">
          <div className="rounded-xl border border-surface-border bg-surface-tertiary/60 p-4">
            <p className="text-xs font-semibold text-text-secondary">Project status</p>
        ...
        <header className="app-header sticky top-0 z-sticky">
          <div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between px-4 sm:px-6 lg:px-10">
            <Link to="/" className="flex items-center gap-3 lg:hidden" aria-label="Sainik School Tracker home">
              <span className="brand-mark" aria-hidden="true">S</span>
              <span className="text-sm font-semibold text-text-primary">Sainik School</span>
            </Link>
            <p className="hidden text-sm font-medium text-text-tertiary lg:block">{headerLabel}</p>
            <div className="flex items-center gap-2">
              <span className="hidden items-center gap-2 rounded-full border border-surface-border bg-surface-tertiary/60 px-3 py-1.5 text-xs font-medium text-text-tertiary lg:flex">
                <Activity className="h-3.5 w-3.5 text-semantic-success-main" aria-hidden="true" />
                Live workspace
              </span>
              <ThemeToggle />
            </div>
          ...
          {mobileNavigationOpen && (
            <nav id="mobile-navigation" className="border-t border-surface-border px-4 py-3 lg:hidden" aria-label="Primary navigation">
              <div className="grid grid-cols-3 gap-2">
                {navItems.map((item) => (
                  <Link
                    ...
                    className={`rounded-lg px-2 py-2 text-center text-xs font-semibold ${location.pathname === item.path ? "bg-primary-600 text-text-primary" : "text-text-secondary hover:bg-surface-tertiary/60 hover:text-text-primary"}`}
                  >
                    {item.label}
                  </Link>
                ))}
